...
void AutoTable::Analysis(const char* szFile)
{
    ...
    string strIncMacro;
    for (vector<struTable>::iterator iter = vecTable.begin(); iter != vecTable.end(); iter++)
    {
        strInc = "";
        strSrc = "";
...
int32 AutoTable::SplitStringByEnter(const string &strInput, vector<string> &vecContent)
{
    ...
    uint64 ullPos, ullHead = 0, ulltail = 0;
    int32 loopstus = 1;
    string strLine;
    while(loopstus)
    {
        ulltail = strInput.find("\r\n", ullHead);
...
int32 AutoTable::GetTableFromVec(vector<string> &vecContent, vector<struTable> &vecTable)
{
    if (vecContent.empty())
    {
        ERRORLOG("in put empty");
    ...
    for (uint32 iIndex = 0; iIndex < vecContent.size(); iIndex++)
    {
        ...
        else if (1 == iStatus && ('`' == strLine[0]))
        {
            ...
            else if ((string::npos != (ullTail = strLine.find("datetime", ullHead))) && (ullTail < ullHead + 10))
            {
            ...
            map<string, struVarInfo>::iterator iter = m_mapVarInfo.find(stTempRow.strType);
...
int32 AutoTable::AnalysisTable(const struTable &stTable, string &strOutPutInc, string &strOutPutSrc)
{
...
void AutoTable::CreateDefaultConstructor(const struTable &stTable, string &strOutPutInc, string &strOutPutSrc)
{
    char szFun[FUNCTION_LENGTH] = {0};
    ...
    strOutPutSrc += szFun;
    for (vector<struRow>::const_iterator iter = stTable.vecRow.begin(); iter != stTable.vecRow.end(); iter++)
    {
...
void AutoTable::CreateCopyConstructor(const struTable &stTable, string &strOutPutInc, string &strOutPutSrc)
{
    char szFun[FUNCTION_LENGTH] = {0};
...
void AutoTable::CreateDestruct(const struTable &stTable, string &strOutPutInc, string &strOutPutSrc)
{
    char szFun[FUNCTION_LENGTH] = {0};
...
void AutoTable::CreateGetRowName(const struTable &stTable, string &strOutPutInc, string &strOutPutSrc)
{
    char szFun[FUNCTION_LENGTH] = {0};
    for (vector<struRow>::const_iterator iter = stTable.vecRow.begin(); iter != stTable.vecRow.end(); iter++)
    {
        memset(szFun, 0, sizeof(szFun));
        snprintf(szFun, sizeof(szFun), "    inline const char* col_%s() {return \"%s\";}\n", iter->strRowName.c_str(), iter->strRowName.c_str());
        strOutPutInc += szFun;
    }
}

void AutoTable::CreateTableName(const struTable &stTable, string &strOutPutInc, string &strOutPutSrc)
{
    char szFun[FUNCTION_LENGTH] = {0};

    snprintf(szFun, sizeof(szFun), "    inline const char* table_name() {return \"%s\";}\n", stTable.strTable.c_str());
    strOutPutInc += szFun;
}

void AutoTable::CreateSetRowValue(const struTable &stTable, string &strOutPutInc, string &strOutPutSrc)
{
    char szFun[FUNCTION_LENGTH] = {0};
    uint32 uiIndex = 0;
    ...
    for (vector<struRow>::const_iterator iter = stTable.vecRow.begin(); iter != stTable.vecRow.end(); iter++)
    {
        ...
        else
        {
            ...
            memset(szFun, 0, sizeof(szFun));
            snprintf(szFun, sizeof(szFun), "bool %s::set_%s(const %s &value)\n{\n    if (value.size() > %u) return false;\n    %s%s = value;\n    m_ui_has_bit |= 0x%08x;\n    return true;\n}\n\n",
                stTable.strTable.c_str(), iter->strRowName.c_str(), iter->stVarInfo.strVarType.c_str(),
                iter->uiLength, iter->stVarInfo.strVarPre.c_str(), iter->strRowName.c_str(), 1 << uiIndex);
            strOutPutSrc += szFun;
...
void AutoTable::CreateGetRowValue(const struTable &stTable, string &strOutPutInc, string &strOutPutSrc)
{
    char szFun[FUNCTION_LENGTH] = {0};
    for (vector<struRow>::const_iterator iter = stTable.vecRow.begin(); iter != stTable.vecRow.end(); iter++)
    {
...
void AutoTable::CreateHas(const struTable &stTable, string &strOutPutInc, string &strOutPutSrc)
{
    char szFun[FUNCTION_LENGTH] = {0};
    int32 uiIndex = 0;
    for (vector<struRow>::const_iterator iter = stTable.vecRow.begin(); iter != stTable.vecRow.end(); iter++)
...
void AutoTable::CreateClearHas(const struTable &stTable, string &strOutPutInc, string &strOutPutSrc)
{
    char szFun[FUNCTION_LENGTH] = {0};
    int32 uiIndex = 0;
    for (vector<struRow>::const_iterator iter = stTable.vecRow.begin(); iter != stTable.vecRow.end(); iter++)
...
void AutoTable::CreateOperatorEqual(const struTable &stTable, string &strOutPutInc, string &strOutPutSrc)
{
    char szFun[FUNCTION_LENGTH] = {0};
...
void AutoTable::CreateGetCount(const struTable &stTable, string &strOutPutInc, string &strOutPutSrc)
{
    char szFun[FUNCTION_LENGTH] = {0};

    snprintf(szFun, sizeof(szFun), "    inline uint32 get_count() {return %ld;}\n", stTable.vecRow.size());
    strOutPutInc += szFun;
}

void AutoTable::CreateSerialize(const struTable &stTable, string &strOutPutInc, string &strOutPutSrc)
{
    char szFun[FUNCTION_LENGTH] = {0};
...
void AutoTable::CreateClassVariables(const struTable &stTable, string &strOutPutInc, string &strOutPutSrc)
{
    char szFun[FUNCTION_LENGTH] = {0};
    strOutPutInc += "    uint32 m_ui_has_bit;\n";
...
void AutoTable::Init()
{
    AddVarInfo("tinyint", "m_c_", "int8");
    AddVarInfo("tinyint unsigned", "m_uc_", "uint8");
    AddVarInfo("smallint", "m_s_", "int16");
    AddVarInfo("smallint unsigned", "m_us_", "uint16");
    AddVarInfo("int", "m_i_", "int32");
    AddVarInfo("int unsigned", "m_ui_", "uint32");
    AddVarInfo("bigint", "m_ll_", "int64");
    AddVarInfo("bigint unsigned", "m_ull_", "uint64");
...
{
    map<string, struVarInfo>::iterator iter = m_mapVarInfo.find(szRowType);
    if (iter == m_mapVarInfo.end())
    {
        struVarInfo stTemp;
